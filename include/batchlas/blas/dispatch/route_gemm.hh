...
inline Route resolve_gemm_route(Route forced, const OpShape& s,
    ...
    if (forced.origin != Origin::Auto) {
        ...
        // A forced native route that cannot serve this shape falls back to the
        // vendor rather than computing nonsense. The old code did the same
        // thing, by returning false from gemm_use_sycl_custom.
    ...
    //
    // The obvious next move -- "take the first merely SUPPORTED route" -- is
    // wrong, and the equivalence test catches it: the order lists Native first,
    // so a 8x8x8 batch-1 GEMM (supported, but far outside the measured window)
    // would select the native kernel where today it goes to the vendor. That
    // silently inverts GEMM's vendor-by-default.
    //
    // Supported-but-not-preferred means "this route is correct but is not the
    // one we measured as better". It should therefore be reached only when
    // there is no vendor left to fall back to -- which is exactly the
    // vendor-off configuration this work package is building toward.
    if (vendor_available) {
        return Route{Origin::Vendor, Algorithm::Auto};
    }
    for (const Route* r = Table::order_begin(); r != Table::order_end(); ++r) {
