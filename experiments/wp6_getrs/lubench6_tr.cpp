...
static int run(const std::string& mode, const char* tn, int n, int nrhs, int batch, int reps) {
    ...
    if (mode == "getrs") {
        ...
        // WP6 getrs perf pass: ONE line added to lubench6.cpp, so this stays the
        // same driver rather than a re-implementation of it (WP4 went 2x off by
        // re-deriving a harness). TRONLY=0|1|2 rotates the array so that the mode
        // the row TIMES -- which is always trs[0] -- can be Trans or ConjTrans.
        // Without it the transposed kernels are exercised for correctness and
        // never timed, and the +1 bank-conflict pad on the diagonal block is a
        // claim about the transposed path alone.
        Transpose trs[3] = {Transpose::NoTrans, Transpose::Trans, Transpose::ConjTrans};
        const char* trn_all[3] = {"N", "T", "C"};
        const char* trn[3] = {"N", "T", "C"};
        {
            const char* e = std::getenv("TRONLY");
            const int sel = e ? std::atoi(e) : 0;
            if (sel >= 1 && sel <= 2) {
                std::swap(trs[0], trs[sel]);
                trn[0] = trn_all[sel]; trn[sel] = trn_all[0];
            }
        }
