...
//   pivsg     ONE SUB-GROUP PER MATRIX, G matrices packed per work-group,
//             argmax by SUB-GROUP SHUFFLE, and NO WORK-GROUP BARRIER anywhere
//             -- the arm this prototype exists to test
//
// WHY pivsg. src/extensions/getrf_cta.cc:174-181 (getrf_leaf_wg) has a hard
// floor of 64 work-items, and the launcher at :228-230 is
// nd_range(batch*wg, wg) -- ONE MATRIX PER WORK-GROUP, always. At n=32,
// batch=8192 that factors a 32-column matrix with >= 64 lanes (over half of them
// idle in the pivot search) and pays THREE WORK-GROUP BARRIERS PER COLUMN where a
// single sub-group needs none. WP6 measured getrf at 0.358x of cuBLAS at n=32 and
// 0.581x at n=64, and the pivot search at 2.42-2.65x over the unpivoted bound --
// worst exactly there.
//
// potrf already does the other thing: potrf_cta.cc:310 `int G = 1; // matrices
// per work-group; > 1 only when L == 32`. getrf never adopted it. This arm is
// that design, isolated, so the idea can be REFUTED before any shipped kernel is
// touched -- this campaign has killed a software-pipelined GEMM k-loop that
// recovered zero time and a cooperative TRSM that lost anyway.
//
// reduce_over_group is deliberately NOT used: WP4 recorded that it ALONE reopens
// the 48 KB launch hole (getrf_cta_device.hh:20-31), and WP6 measured it SLOWER
// than the SLM tree anyway (3.05-3.30x vs 2.42-2.65x over the unpivoted bound).
// This arm uses shift_group_left / group_broadcast over ONE sub-group.
//
...
enum class Var { NoPiv, SwapOnly, PivMan, PivGrp, PivSg };
...
// ---------------------------------------------------------------- pivsg
// One SUB-GROUP per matrix, G matrices per work-group. Every synchronisation is a
// SUB-GROUP barrier; the work-group never synchronises as a whole.
//
// THE BARRIER ARGUMENT, AND WHY IT IS NOT A FREE LUNCH. WP3's trsm shipped a
// WRONG ANSWER from exactly this substitution: a lane-strided SLM write read back
// by a different lane with no barrier, invisible at wg=32 where every test sat,
// and it took a 6.05e+16 relative error at wg=256 to expose it. What makes it
// legitimate HERE is narrower than "sub-groups run in lockstep": every lane that
// writes a word and every lane that reads it are in the SAME sub-group BY
// CONSTRUCTION -- one sub-group owns a matrix and no other sub-group touches its
// tile -- and the sub-group barrier is emitted anyway rather than relying on
// lockstep. If a launch ever put two sub-groups on one matrix this would be the
// WP3 defect again, which is why the sub-group size is pinned by
// reqd_sub_group_size and checked against the device rather than assumed.
template <typename T>
static sycl::event lu_launch_sg(sycl::queue& q, T* A, int* piv_out,
                                int n, int ld, int batch, int sg_size, int Gpack,
                                size_t tile_elems_per_mat) {
    using R = typename RealOf<T>::type;
    const int wg = sg_size * Gpack;
    const int groups = (batch + Gpack - 1) / Gpack;
    return q.submit([&](sycl::handler& h) {
        sycl::local_accessor<T, 1> tile(sycl::range<1>(tile_elems_per_mat * size_t(Gpack)), h);
        h.parallel_for(sycl::nd_range<1>(sycl::range<1>(size_t(groups) * size_t(wg)),
                                         sycl::range<1>(size_t(wg))),
                       [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(32)]] {
            const auto sg = it.get_sub_group();
            const int lane = int(sg.get_local_linear_id());
            const int sgid = int(sg.get_group_linear_id());   // which matrix in this wg
            const size_t b = size_t(it.get_group(0)) * size_t(Gpack) + size_t(sgid);
            if (b >= size_t(batch)) return;                   // ragged last work-group

            T* S = &tile[tile_elems_per_mat * size_t(sgid)];  // this matrix's own slot
            const size_t st = size_t(n) * size_t(n);
            T* Gm = A + b * st;
            int* P = piv_out + b * size_t(n);
            const int W = sg_size;

            for (int e = lane; e < n * n; e += W) {
                const int i = e % n, j = e / n;
                S[size_t(i) + size_t(j) * ld] = Gm[size_t(i) + size_t(j) * n];
            }
            sycl::group_barrier(sg);

            for (int k = 0; k < n; ++k) {
                // ---- argmax over rows k..n-1 by SUB-GROUP SHUFFLE -----------
                // LOWEST index wins the tie -- LAPACK's rule, and the one
                // getrf_cta_device.hh documents.
                R bv = R(-1); int bi = k;
                for (int i = k + lane; i < n; i += W) {
                    const R v = cabs1(S[size_t(i) + size_t(k) * ld]);
                    if (v > bv || (v == bv && i < bi)) { bv = v; bi = i; }
                }
                for (int off = W / 2; off > 0; off >>= 1) {
                    const R ov = sycl::shift_group_left(sg, bv, off);
                    const int oi = sycl::shift_group_left(sg, bi, off);
                    if (ov > bv || (ov == bv && oi < bi)) { bv = ov; bi = oi; }
                }
                const int p = sycl::group_broadcast(sg, bi, 0);

                if (lane == 0) P[k] = p + 1;                  // 1-BASED LAPACK ipiv
                if (p != k) {
                    for (int j = lane; j < n; j += W) {
                        const size_t a0 = size_t(k) + size_t(j) * ld;
                        const size_t a1 = size_t(p) + size_t(j) * ld;
                        const T t = S[a0]; S[a0] = S[a1]; S[a1] = t;
                    }
                ...
                const T d = S[size_t(k) + size_t(k) * ld];
                const T r = crecip(d);
                for (int i = k + 1 + lane; i < n; i += W)
                    S[size_t(i) + size_t(k) * ld] = cmul(S[size_t(i) + size_t(k) * ld], r);
                sycl::group_barrier(sg);

                const int m = n - k - 1;
                for (int e = lane; e < m * m; e += W) {
                    const int i = k + 1 + (e % m), j = k + 1 + (e / m);
                    S[size_t(i) + size_t(j) * ld] =
                        S[size_t(i) + size_t(j) * ld]
                        - cmul(S[size_t(i) + size_t(k) * ld], S[size_t(k) + size_t(j) * ld]);
            ...
            for (int e = lane; e < n * n; e += W) {
                const int i = e % n, j = e / n;
                Gm[size_t(i) + size_t(j) * n] = S[size_t(i) + size_t(j) * ld];
            }
        });
    });
}
...
static int run(const char* vn, const char* tn, int n, int batch, int reps) {
    ...
    // pivsg geometry. One sub-group per matrix, G matrices per work-group.
    // G is capped so wg_size <= 256, which is potrf_cta.cc:396's cap and the
    // same register-pressure argument: the ladder must not go where
    // registers x work-group-size exceeds 65,536.
    const int sg_size = 32;
    int Gpack = 1;
    if (V == Var::PivSg) {
        const size_t per_mat = tile_elems * sizeof(T);
        while (Gpack < 8 && sg_size * (Gpack + 1) <= 256 &&
               per_mat * size_t(Gpack + 1) <= budget) ++Gpack;
        if (const char* ge = std::getenv("GPACK")) Gpack = std::max(1, std::atoi(ge));
    }
    ...
    const size_t slm = (V == Var::PivSg)
        ? tile_elems * sizeof(T) * size_t(Gpack)
        : tile_elems * sizeof(T) + red_slots * (sizeof(R) + sizeof(int)) + sizeof(int);
    ...
    auto once = [&] {
        ...
        if constexpr (V == Var::PivSg) {
            lu_launch_sg<T>(q, dA, dP, n, ld, batch, sg_size, Gpack, tile_elems);
        } else {
...
static int dispatch_var(const std::string& v, const char* tn, int n, int b, int r) {
    ...
    if (v == "pivsg")    return run<T, Var::PivSg>("pivsg", tn, n, b, r);
...
int main(int argc, char** argv) {
    if (argc < 6) {
        std::fprintf(stderr,
            ...
            "variants: nopiv | swaponly | pivman | pivgrp | pivsg\n"
