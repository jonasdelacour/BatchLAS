...
# BATCHLAS_HAS_CUBLAS 0. In this build there is NO getrf/getrs/getri at all
# (WP6 has not been written), so only the composition modes can run here;
# libbatchlas_backends_cuda.so is deliberately absent from the link line.
set -euo pipefail
W=/home/jonaslacour/BatchLAS/.claude/worktrees/vendor-independence-plan
D="$W/experiments/wp6_lu/kernels"
SRC="${1:-$D/luverify.cpp}"
OUT="${2:-$D/luverify_nv}"
