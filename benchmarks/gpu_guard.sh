...
# using it.
#
#     benchmarks/gpu_guard.sh 1 ./factor_bench potrf float 32 32 0 8192 7
#
# Ported unchanged in behaviour from experiments/gpu_guard.sh at the tag
# perf-evidence/vendor-independence, which is the script every number in
# docs/perf was measured through.
#
...
#
# Exits non-zero rather than reporting anything it cannot trust:
#   2 usage   3 cannot query the GPU   4 still busy after MAX_WAIT
#   5 a foreign process appeared DURING the run -- discard the numbers
set -uo pipefail
...
# The SM clock at the start of the run. A cold card sits at ~210 MHz and a warm
# one at ~2600; a cell measured from cold is not comparable with one measured
# hot, which is why the harness also warms for WARM_S before it times anything.
