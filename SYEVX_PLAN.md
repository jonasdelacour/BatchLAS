...
**§7.5 soft locking** is implemented as column masking but **defaults to off** —
it was measured to cost 0–1 iterations and save none, and the regime where it
should pay (Cholesky-based `ortho`) is unreachable on a CPU-only build, which
forces Householder. `BATCHLAS_SYEVX_SOFT_LOCK=1` enables it for a GPU A/B.

**§7.10 Jacobi** landed; its Chebyshev half did not.
