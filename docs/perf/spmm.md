...
`kSpmmOrder` (`include/batchlas/blas/dispatch/route_spmm.hh:175-178`) has exactly two entries:
...
Body selection is in the launcher, on `transA` (`spmm_native.cc:566-611`, under the note at `:560-565`), deliberately below the
...
(`route_env.hh:76-99`): `native:direct`, `native`, `direct`, `vendor`, `auto`. A misspelling is **silent** —
...
`route_spmm.hh:292-427`, stripped of its ~140 lines of evidence comments:
...
`route_spmm.hh:190-259`. Correctness gates only: CSR format (`:198`); no heterogeneous *dense* batch (`:211` — a CSR view is never
...
* **No `nnz` field on `SpmmShape`** (`route_spmm.hh:132-141`). `MatrixView::nnz()` is the per-item *capacity* (the batch maximum);
...
* **Event timing is deliberately not installed.** `make_event_timed_kernel_ms` costs a recorded ~0.36 ms per call
  (recorded at `VENDOR_INDEPENDENCE_PLAN.md:1949`; `spmm_benchmark.cc:47` cites this section back); cell L's
...
**Mechanism, hypothesis only, not confirmed by profile**: `kNCmax<Cx<float>>` is 8 (`spmm_native.cc:88-92`), so `nrhs=25` needs
...
Every refuting cell in that table is on the **scattered** pattern (`pat=1`), the worst one included. `route_spmm.hh:340-341` and
...
(`run_all.sh:34`, `LOBPCG_ARGS="... 0 0 1"`; `spmm_benchmark.cc:142-143` fixes `kBanded = 0` / `kRandom = 1`, and `:99` says so in
...
(2^30), and the scatter's own range guard (`spmm_native.cc:541-542`) `continue`s **before** `av` is multiplied — so both halves of
...
**native** (`tests/spmm_tests.cc:508-512`: `pin_text` containing `native`, or equal to `direct`/`cta`/`blocked`), so pinned-route
...
* **Three in-tree comments are stale or wrong, and all three are the kind a reader trusts.** (a)
  `src/dispatch/entry_points/sparse.cc:98-110`'s route-neutrality comment states that `preferred()` "is false for every route, every
  type and every shape (route_spmm.hh:271)" and derives byte-identical vendor routing from that; the shipped `preferred()` is not
  ...
  says the opposite. (b) `tests/spmm_tests.cc:92` says the transposed refusal-skip is "DISABLED when `BATCHLAS_SPMM_ROUTE` is set";
  the code at `:508-512` disables it only for a **native** pin, and the loose form is what turned 92 NETLIB skips into 92 failures.
  (c) `route_spmm.hh:340-341` (and the plan's negative-results list) call the worst transposed cell "banded"; `verdict.txt` and the
