...
|---|---|---|
| `geqrf` | `{Native, CTA}`, `{Native, Blocked}`, `{Vendor, Auto}` (`route_geqrf.hh:195-199`) | **false everywhere** (`route_geqrf.hh:363-367`) |
| `orgqr` | `{Native, Blocked}`, `{Vendor, Auto}` (`route_orgqr.hh:108-111`) | **false everywhere** (`route_orgqr.hh:231-235`) |
| `ormqr` | `{Native, Blocked}`, `{Vendor, Auto}` (`route_ormqr.hh:45-48`) | `is_native(r) && supports(r, s)` (`route_ormqr.hh:77-79`) |

`geqrf` and `orgqr` ship **route-neutral**: a vendor-present build takes cuSOLVER for every shape. The kernels are reachable only from a vendor-free build (`route_resolve.hh:113-127`), from `BATCHLAS_GEQRF_ROUTE` / `BATCHLAS_ORGQR_ROUTE`, or from the direct entry points `geqrf_cta_dispatch` / `geqrf_blocked_dispatch` / `orgqr_blocked_dispatch`. The 3.24x (`geqrf`) and 7.85x (`orgqr`) geomeans below are therefore **unrealised in the default build** — debt 1.
...
`supports()` for both new tables is correctness-only. Gates that matter: `m >= n` for both `geqrf` native arms (handed a wide view the trailing update walks past the bottom of the panel, `route_geqrf.hh:217-232`); `n <= m` for `orgqr` (Q's columns live in C^m, `route_orgqr.hh:146-157`); GPU-only for both; and heterogeneous batch for both, refused because nothing in this tree gets heterogeneous-batch QR right (netlib included — `netlib_lapack.cc:1430-1442` hoists m and n out of its loop, and its `orgqr` at :1472-1477 hoists m, n and k).
...
The shipped `geqrf` window, verbatim (`route_geqrf.hh:443-473`):
...
**The `route_gemm.hh` `batch >= 64` gate (`route_gemm.hh:122`) costs the vendor build.** At N=2048 b=32 six `double` trailing-GEMM cells go to cuBLAS and the vendor build is **1.14–1.25x slower** than the vendor-free build on the same shapes. Small, but a real `preferred()` edge on a shape `geqrf` issues.
