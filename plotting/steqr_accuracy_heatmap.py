...
def _prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "relerr" not in df.columns and "max_relerr" in df.columns:
        df["relerr"] = df["max_relerr"]
...
def plot_heatmap(
    ...
    *,
    ...
    success_df = df[np.isfinite(df["log10_relerr"].to_numpy())].copy()
    if success_df.empty:
        raise ValueError("No successful rows (finite log10_relerr) matched filters")
    xedges = _make_bins(df["log10_cond"].to_numpy(), x_bins, clamp=clamp_x)
    yedges = _make_bins(success_df["log10_relerr"].to_numpy(), y_bins, clamp=clamp_y)

    H = _hist2d(success_df, xedges=xedges, yedges=yedges)
    H = _normalize_histogram(H, xedges, yedges)
    fcenters, frates = _bin_failure_rate(df["log10_cond"].to_numpy(), df["log10_relerr"].to_numpy(), xedges)
...
def plot_multi_heatmap(
    ...
    *,
    ...
    success_df = df[np.isfinite(df["log10_relerr"].to_numpy())].copy()
    if success_df.empty:
        raise ValueError("No successful rows (finite log10_relerr) matched impl/N filters")
...
def main() -> None:
    ...
    parser.add_argument("--clamp-y", default=None, help="clamp log10(relerr) to min,max (e.g. --clamp-y=-10,0)")
    ...
    parser.add_argument("--clamp-y-min", type=float, default=None, help="minimum clamp for log10(relerr)")
    parser.add_argument("--clamp-y-max", type=float, default=None, help="maximum clamp for log10(relerr)")
