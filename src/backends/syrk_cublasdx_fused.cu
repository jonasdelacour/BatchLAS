...
inline bool has_max_alignment(const SyrkLaunchDescriptor& desc) {
    return is_16b_aligned(desc.a_ptr) && is_16b_aligned(desc.c_ptr) &&
        (desc.lda % 4) == 0 && (desc.ldc % 4) == 0 &&
        (desc.stride_a % 4) == 0 && (desc.stride_c % 4) == 0;
}
...
cudaError_t dispatch_pipelined_family_for_current_sm(const SyrkLaunchDescriptor& desc, cudaStream_t stream) {
    ...
    switch (prop.major * 100 + prop.minor * 10) {
    case kSupportedSM:
        return launch_pipelined_for_sm<kSupportedSM, TileM, TileN, TileK, FillMode, TransMode>(desc, stream);
    default:
        return cudaErrorNotSupported;
    }
}
