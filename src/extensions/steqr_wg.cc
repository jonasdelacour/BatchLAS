...
Event steqr_wg_impl(Queue& ctx,
                    ...
                    const Span<std::array<int32_t, 3>>& deflation_indices,
                    const Span<ApplyOrder>& order_view,
                    BumpAllocator allocator,
                    size_t max_sweeps,
                    T zero_threshold) {
    BATCHLAS_KERNEL_TRACE_SCOPE("steqr_wg");
    auto n = d.size();
    ...
    auto temp_deflation_indices = VectorView<std::array<int32_t, 2>>(
        allocator.allocate<std::array<int32_t, 2>>(ctx, VectorView<std::array<int32_t, 2>>::required_span_length(n / 2, 1, n / 2, batch_size)),
        n / 2,
        batch_size,
        1,
        n / 2);
    ...
    {
        BATCHLAS_KERNEL_TRACE_SCOPE("steqr_wg:deflation_ranges");
    ...
    {
        BATCHLAS_KERNEL_TRACE_SCOPE("steqr_wg:deflation_writeout");
        ctx->submit([&](sycl::handler& cgh) {
            cgh.parallel_for(sycl::range(batch_size), [=](sycl::id<1> id) {
                auto i = id[0];
    ...
    {
        BATCHLAS_KERNEL_TRACE_SCOPE("steqr_wg:francis_sweep");
        ctx->submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<FrancisKernel<T>>(sycl::nd_range(sycl::range(ncus * 128), sycl::range(64)), [=](sycl::nd_item<1> item) {
                ...
                for (int gid = i; gid < n_problems; gid += item.get_global_range(0)) {
                    ...
                    auto n_local = end_ix - start_ix;

                    if (n_local == 1) continue;
                    if (n_local == 2) {
                        if (store_givens) {
                    ...
                    for (size_t k = 0; k < max_sweeps; ++k) {
                        ...
                        for (size_t idx = 0; idx < n_local - 1; ++idx) {
                            anorm = std::fmax(anorm, std::fmax(std::abs(d_(idx)), std::abs(e_(idx))));
                        }

                        if (anorm > internal::ssfmax<T>()) {
                            auto alpha = internal::ssfmax<T>() / anorm;
                            for (size_t idx = 0; idx < n_local; ++idx) d_(idx) *= alpha;
                            for (size_t idx = 0; idx < n_local - 1; ++idx) e_(idx) *= alpha;
                        ...
                        auto d_get = [&](size_t idx) -> T {
                            return QR ? d_(idx) : d_(n_local - 1 - idx);
                        };
                        auto e_get = [&](size_t idx) -> T {
                            return QR ? e_(idx) : e_(n_local - 2 - idx);
                        };

                        const auto shift = wilkinson_shift(d_get(n_local - 2),
                                                           e_get(n_local - 2),
                                                           d_get(n_local - 1));
...
Event steqr_wg(Queue& ctx,
    ...
    auto givens_rotations = jobz == JobType::EigenVectors
                                ? MatrixView<std::array<T, 2>>(pool.allocate<std::array<T, 2>>(ctx, stride * max_subproblems * batch_size).data(),
                                                                n - 1,
                                                                n_sweeps_to_store,
                                                                n - 1,
                                                                stride,
                                                                max_subproblems * batch_size)
...
size_t steqr_wg_buffer_size(Queue& ctx,
    ...
    size_t size = BumpAllocator::allocation_size<T>(ctx, d_size)
                ...
                + BumpAllocator::allocation_size<std::array<int32_t, 3>>(ctx, batch_size * (n / 2 + 1))
                + BumpAllocator::allocation_size<int32_t>(ctx, batch_size)
                + BumpAllocator::allocation_size<std::array<int32_t, 2>>(ctx, VectorView<std::array<int32_t, 2>>::required_span_length(n / 2, 1, n / 2, batch_size));
    if (jobz == JobType::EigenVectors) {
    ...
    size += sort_buffer_size<T>(ctx,
                                eigenvalues.data(),
                                MatrixView<T, MatrixFormat::Dense>(nullptr, d.size(), d.size(), d.size(), d.size() * d.size(), d.batch_size()),
                                jobz);
    return size;
}

#if BATCHLAS_HAS_CUDA_BACKEND
template Event steqr_wg<Backend::CUDA, float>(Queue&, const VectorView<float>&, const VectorView<float>&, const VectorView<float>&, const Span<std::byte>&, JobType, SteqrParams<float>, const MatrixView<float, MatrixFormat::Dense>&);
template Event steqr_wg<Backend::CUDA, double>(Queue&, const VectorView<double>&, const VectorView<double>&, const VectorView<double>&, const Span<std::byte>&, JobType, SteqrParams<double>, const MatrixView<double, MatrixFormat::Dense>&);
#endif

#if BATCHLAS_HAS_HOST_BACKEND
template Event steqr_wg<Backend::NETLIB, float>(Queue&, const VectorView<float>&, const VectorView<float>&, const VectorView<float>&, const Span<std::byte>&, JobType, SteqrParams<float>, const MatrixView<float, MatrixFormat::Dense>&);
template Event steqr_wg<Backend::NETLIB, double>(Queue&, const VectorView<double>&, const VectorView<double>&, const VectorView<double>&, const Span<std::byte>&, JobType, SteqrParams<double>, const MatrixView<double, MatrixFormat::Dense>&);
#endif

template size_t steqr_wg_buffer_size<float>(Queue&, const VectorView<float>&, const VectorView<float>&, const VectorView<float>&, JobType, SteqrParams<float>);
template size_t steqr_wg_buffer_size<double>(Queue&, const VectorView<double>&, const VectorView<double>&, const VectorView<double>&, JobType, SteqrParams<double>);
