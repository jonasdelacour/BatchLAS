...
// The three entry points are always instantiated as a set, so bind them into a
// single leaf for the backend x scalar-type driver. The leaf takes its type
// parenthesised, which is what BATCHLAS_FOR_EACH_SCALAR_TYPE_1 hands it.
#define SYTRD_BAND_REDUCTION_ALL_INSTANTIATE(back, fp) \
    SYTRD_BAND_REDUCTION_INSTANTIATE(back, BATCHLAS_UNPAREN fp) \
    SYTRD_BAND_REDUCTION_BUFFER_PARAMS_INSTANTIATE(back, BATCHLAS_UNPAREN fp) \
    SYTRD_BAND_REDUCTION_SINGLE_STEP_INSTANTIATE(back, BATCHLAS_UNPAREN fp)

BATCHLAS_INSTANTIATE_SCALAR_ALL_BACKENDS(SYTRD_BAND_REDUCTION_ALL_INSTANTIATE)
