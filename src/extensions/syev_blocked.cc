...
// The panel block size the tuning harness produced is float-only, and for
// 256 < n <= 512 it is badly wrong for complex.
//
// tuning::sytrd_block_size_for_n returns 8 in that bucket. 8 is right for float
// (measured below: nb=16 beats it by only 1.04x, inside the 1.10x neutral band)
// but a complex panel column moves twice the bytes and does four times the
// flops per element, so it wants a much wider panel to amortise the
// O(n) barrier-and-reduction chain that latrd_lower_panel runs per column.
//
// MEASURED 2026-08-07, RTX 4090 device 1, complex<float>, eigenvectors,
// BLOCKED provider, us/matrix, median of 5, one process on the device:
//
...
//
// nb = 32 is the optimum or within 1.5% of it at every n in the bucket, and it
// beats the committed 8 by 1.16x - 1.20x. For float the same sweep gives
// 327.31 / 313.92 / 315.96 / 320.83 / 340.21 at n=512 (nb 8/16/24/32/48), i.e.
// a 1.04x spread -- neutral, so float keeps the harness value untouched.
//
// The other buckets were measured too and are NOT changed: n <= 128 prefers 8
// (nb=8 vs 16 is 2.36 vs 2.37 at n=64, 5.92 vs 6.07 at n=96 -- noise);
// 128 < n <= 256 prefers the committed 16 (n=192: 33.83 at 16 vs 36.09 at 8);
// n > 512 is within 1.03x of the committed 48 at nb=32, below the neutral band.
//
// This lives at the consumer rather than in tuning_params.hh on purpose: that
// header is regenerated by evaluation/tuning/generate_tuning_header.py from a
// float-only bench, so a complex bucket written there would be silently
...
BATCHLAS_INSTANTIATE_SCALAR_ALL_BACKENDS(SYEV_BLOCKED_INSTANTIATE)
